...
REQUIRED_INPUTS = {
    ...
    "nav_get_speed": ("robonix/service/navigation/get_speed_limit", "mcp"),
    "nav_set_speed": ("robonix/service/navigation/set_speed_limit", "mcp"),
}


def resolve_inputs(deadline_s: float = 60.0) -> dict[str, str]:
    """Resolve the existing map and navigation contracts within a bounded startup window."""
...
def init(cfg):
    """Validate package selection settings before allocating runtime resources."""
    global selection_config
    try:
        selection_config = parse_config(cfg)
...
def activate():
    ...
    ctrl = ExploreController(
        ...
        nav_get_speed_endpoint=inputs["nav_get_speed"],
        nav_set_speed_endpoint=inputs["nav_set_speed"],
...
def main() -> int:
    """Run the skill and release its controller when the process stops."""
    explore_skill.run()
    if ctrl is not None:
        ctrl.stop_runtime()
    return 0
...
@explore_skill.on_shutdown
def shutdown():
    """Complete navigation cancellation and speed cleanup before exiting the skill."""
    return deactivate()
