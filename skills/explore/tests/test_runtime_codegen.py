...
class RuntimeCodegenTest(unittest.TestCase):
    def _fixture(self, root: Path) -> tuple[Path, dict[str, str]]:
        """Create isolated shell tools that exercise packaging without Docker or ROS."""
        package = root / "package"
        fake_bin = root / "bin"
        runtime_proto = root / "runtime-proto"
        package.mkdir()
        (package / 'scripts').mkdir()
        (package / 'scripts' / 'stop.sh').write_text((ROOT / 'scripts' / 'stop.sh').read_text())
        fake_bin.mkdir()
        runtime_proto.mkdir()
    ...
    def test_wrapper_generates_offline_and_masks_only_runtime_stubs(self) -> None:
        """Verify runtime stubs, safe mounts, and graceful replacement of an old container."""
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("stop --time 120", log)
    ...
    def test_failed_codegen_preserves_previous_good_output(self) -> None:
        """A failed generation preserves the previous runnable protobuf directory."""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            wrapper, env = self._fixture(root)
            previous = (
