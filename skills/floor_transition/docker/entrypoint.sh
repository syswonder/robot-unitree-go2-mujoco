...
# floor_transition_rbnx container entrypoint.
#
# Single long-running process: floor_transition_skill.atlas_bridge runs an
# rclpy node, registers 3 MCP capabilities with atlas, and serves
# them via FastMCP HTTP. The skill's task lifecycle (stair traversal,
# landing alignment, and floor-map switching) is internal to that
# process; map / nav are atlas-resolved.
set -eo pipefail

source /opt/ros/humble/setup.bash
if [[ -f /floor_transition/rbnx-build/codegen/ros2_idl/install/setup.bash ]]; then
    source /floor_transition/rbnx-build/codegen/ros2_idl/install/setup.bash
fi
...
PROTO_GEN=/floor_transition/rbnx-build/codegen/proto_gen
if [ ! -f "$PROTO_GEN/atlas_pb2.py" ] \
    || [ ! -f "$PROTO_GEN/floor_transition_pb2.py" ] \
    || [ ! -f "$PROTO_GEN/robonix_contracts_pb2_grpc.py" ]; then
    echo "[entrypoint] missing runtime-compatible protobuf stubs in $PROTO_GEN; rebuild and restart floor_transition" >&2
    exit 1
fi

export PYTHONPATH="/floor_transition:/floor_transition/rbnx-build/codegen/proto_gen:/floor_transition/rbnx-build/codegen/robonix_mcp_types:${PYTHONPATH:-}"
if [ -d /robonix-api ]; then
    export PYTHONPATH="/robonix-api:${PYTHONPATH}"
fi

mkdir -p /floor_transition/rbnx-build/data
...
# container's stdout and `rbnx logs -t com.robonix.skill.floor_transition` showed
# nothing. rbnx already tags each component's output, so no prefix is needed.
exec python3 -u -m floor_transition_skill.atlas_bridge
