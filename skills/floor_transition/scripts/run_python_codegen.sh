...
# Generate FloorTransition's Python protobuf stubs with versions compatible with the
...
if ! command -v uv >/dev/null 2>&1; then
    echo "[floor_transition/codegen] error: 'uv' not found on PATH. Install: https://docs.astral.sh/uv/" >&2
    exit 1
fi
...
if [[ "$compatible" != "1" ]]; then
    echo "[floor_transition/codegen] preparing protobuf $PROTOBUF_VERSION / grpcio-tools $GRPC_TOOLS_VERSION / grpcio $GRPCIO_VERSION"
    rm -rf "$VENV"
    uv venv "$VENV"
...
echo "[floor_transition/codegen] rbnx codegen -p $PKG $*"
...
if actual != expected:
    raise RuntimeError(f"FloorTransition protobuf environment mismatch: {actual} != {expected}")

import atlas_pb2_grpc  # noqa: F401, E402
import floor_transition_pb2  # noqa: F401, E402
import floor_transition_mcp  # noqa: F401, E402
import robonix_contracts_pb2_grpc  # noqa: F401, E402

print(
    "[floor_transition/codegen] generated protobuf, gRPC, and MCP imports OK "
